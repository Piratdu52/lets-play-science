...
- **g** Défininition successive: Il serait possible de faire des définitions d'un concept, qui pouront être utilisé par la suite de sorte à afficher les postes le contenenant, fournir un lien de type Wiki sur le concept dédié au projet. Cela peut mettre en avant des postes s'appuyant sur la communauté grace a un algorithme de type PageRank (https://fr.wikipedia.org/wiki/PageRank) et permetre la genération automatique de carte mentales où la taille des concept varira en fonction de leur redondance et avec des liens spécialisé si on des balises sémantique dans la définition. Par exemple sur une de définition du concept de Chat: "Le chat est un <abstraction>Felin</abstraction>. Il en exite plusieur espece dont la plus chére est le <specialisation>Siamois</specialisation>".
